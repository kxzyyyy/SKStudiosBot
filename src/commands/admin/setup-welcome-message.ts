...
export default {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Customize the welcome message embed'),
  ...
  async execute(interaction: any) {
    ...
    // Step 3: Color
    await channel.send({ content: '**Step 3:** What color would you like? (hex format, e.g. `#00ff00`)' });
    ...
    if (!/^#[0-9A-Fa-f]{6}$/.test(colorInput)) {
      await channel.send({ embeds: [createErrorEmbed('Error', 'Invalid color format. Please use hex format like `#00ff00`.')] });
      return;
    }
    ...
    await configService.updateConfig({
      ...
      welcomeFooterImage: footerImage
    });

    await channel.send({ embeds: [createSuccessEmbed('Welcome Message Updated', 'The welcome message has been customized successfully.')] });
